function [refSeq, caseSeq, testSeq, test2Seq, idxCase] = randomSampleCaseRefTest(hapSeq, nCase, nRef, nTest)
...
totalSample = nCase + nRef + nTest + nTest;
...
idxTest2 = sort([idx(nCase+nRef+nTest:end)*2; idx(nCase+nRef+nTest:end)*2-1]);

caseSeq = hapSeq(idxCase, :);
refSeq = hapSeq(idxRef, :);
testSeq = hapSeq(idxTest, :);
test2Seq = hapSeq(idxTest2, :);
end