...
indxAllS = 1:nSample;
indxAllR = 1:nRef;
indxAllS = [1:2:nSample 2:2:nSample];
indxAllR = [1:2:nRef 2:2:nRef];

halfSample = floor(nSample/2);
halfRef = floor(nRef/2);
seqS = seqSample(indxAllS(1:halfSample)); % sample group 
seqR = seqRef(indxAllR(1:halfRef)); % reference group
seqSOther = seqSample(indxAllS(halfSample+1:end)); % the rest is used as test group 
...
plot(indxSOther,  Trall(indxSOther), '.b');

if exists(indxROther)
