...
function [] = RELAX_HFREQ_HBSTART_WTCCC1_1500_Genotype_Recombo()
...
%     plotCoverRate(totalRefFreqInfo, 'total freq info');
    ...
    %% KEEP hight frequency genotypes in the reference
    [freqSampledGenoSeq] = highFreqStaySample(refSeq, wtccc1Conf.caseSize, wtccc1Conf.highFreqT, wtccc1Conf.blocks);
    
    %% doing innerblock learning to approach the frequency
    fprintf(wtccc1Conf.logfid, 'start inner block learning');
    [randomCaseSeq] = gInnerSeqLearning(caseSeq, freqSampledGenoSeq, wtccc1Conf);
